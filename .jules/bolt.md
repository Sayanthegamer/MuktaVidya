...
## 2024-06-03 - Optimize Base64 MIME Type Extraction
...
## 2024-06-20 - Intersection Observer Object Caching
**Learning:** Instantiating `IntersectionObserver` within a `useEffect` dependent on toggled state (e.g., `isStreaming`) causes the observer to be repeatedly created and destroyed. While browsers handle object instantiation and garbage collection well, repeated creation on state toggles within interactive components introduces minor, unnecessary overhead, especially when combined with rapid DOM events or frequent API streaming start/stop boundaries.
**Action:** Retain `IntersectionObserver` instances across component lifecycles using `useRef`. Initialize the observer lazily within a `useEffect` once, and simply call `.observe()` and `.disconnect()` on target nodes according to conditional logic (like pausing observations during streaming and resuming afterward).
