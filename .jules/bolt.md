...
## 2024-06-03 - useImageSolver Redundant State
**Learning:** Found that `useImageSolver.ts` maintained `imagePreview` and `solution` as independent `useState` hooks while simultaneously building an array of `messages` containing the same data. This duplication required multiple `setState` calls during asynchronous processes, risking race conditions and breaking single-source-of-truth principles.
**Action:** Removed duplicate state definitions. Derived these legacy properties explicitly by indexing into the `messages` array instead. Simplified the hook API response, lowering maintenance burden and potential UI synchronization errors.
