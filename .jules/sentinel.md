...
## 2026-06-07 - Add payload semantic validation to Feedback API
**Vulnerability:** The `/api/feedback` route parsed incoming JSON payloads without enforcing structural integrity or property limits. An attacker could potentially pass logically invalid structures (such as arrays instead of objects) or string inputs of unbounded lengths up to the absolute `MAX_BODY_BYTES` limit. This introduces application-layer resource consumption and logic issues.
**Learning:** Checking for JSON syntax validity is not enough. Without explicitly defining and enforcing a strict semantic schema (verifying properties exist, validating data types, and asserting max string lengths), applications remain vulnerable to malformed payloads which could later cause database anomalies or excessive processing time.
**Prevention:** Always combine byte-level boundaries with strong semantic validation rules on parsed JSON bodies. Validate object shapes explicitly, enforce string lengths, and strictly enumerate allowed type values.
