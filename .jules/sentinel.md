...
## 2025-06-03 - Type Errors in JSON Array/Object Payloads
**Vulnerability:** The `/api/solve` endpoint assumed parsed JSON payload properties `msg.text` and `msg.imageBase64` were strings if they were truthy. A malicious actor could supply an array or object (`"text": ["malicious"]`), bypassing length constraints and causing unhandled `TypeError` exceptions (`msg.text.trim is not a function`) that leaked stack details and caused server 500 crashes.
**Learning:** TypeScript interfaces do not validate data at runtime. Truthy checks do not guarantee data type correctness. Calling string methods on loosely typed JSON inputs causes application crashes.
**Prevention:** Always execute strict `typeof x !== "string"` checks before calling string prototype methods like `trim()` or `.length` on user-supplied JSON values.
