...
## 2026-06-07 - Add strict type checking to JSON payload structures
**Vulnerability:** The `/api/solve` route validated the length of array items and string contents but assumed that `msg.text` and `msg.imageBase64` were strings without explicit `typeof` checks. A malformed payload containing arrays or objects instead of strings (e.g. `messages: [{text: []}]`) would pass structural checks but cause unhandled `TypeError` exceptions (e.g., calling `.trim()` on an array), leading to unexpected 500 Internal Server Errors.
**Learning:** Checking for truthiness or length does not guarantee the correct primitive type when parsing untyped JSON. Attempting string operations on unexpected nested JSON structures can crash the request handler.
**Prevention:** Always enforce strict `typeof` checks on properties extracted from `JSON.parse` before invoking methods like `.trim()` or `.startsWith()`.
