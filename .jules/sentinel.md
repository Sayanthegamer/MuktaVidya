...
## 2025-06-07 - Event Loop Blocking via Base64 string manipulation
**Vulnerability:** The `/api/solve` route was parsing multimegabyte Base64 image payload strings and executing `.trim()` on them to check for truthiness: `msg.imageBase64.trim().length > 0`. Because strings in JavaScript are immutable, `.trim()` creates a duplicate string in memory. More importantly, string manipulation on massive payloads is synchronous and blocking, resulting in Node.js Event Loop starvation (Application-level DoS).
**Learning:** Checking for empty massive payload strings using string manipulation functions like `.trim()` or RegEx causes synchronous memory duplication and event loop stalls, opening up denial of service vulnerabilities.
**Prevention:** Never perform `.trim()` or synchronous string manipulation operations on massive Base64 or unstructured data payloads. Use simple truthiness logic or `length` checks (`msg.imageBase64.length > 0`) for massive strings, as these check metadata without traversing the whole character array.
